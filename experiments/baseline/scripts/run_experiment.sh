#!/bin/bash
# Polymer Prediction Baseline Experiment実行スクリプト
# 
# 使用方法:
...
# 実験実行
run_experiment() {
    echo "🚀 Polymer Prediction Baseline Experiment実行開始"
    ...
    # 実験実行
    echo "▶️  実験スクリプト実行中..."
    python3 "$SCRIPT_DIR/local_polymer_prediction.py"
    
    # 結果表示
    echo ""
    echo "🎉 実験完了!"
    echo "📊 結果は以下のディレクトリに保存されました:"
    latest_experiment=$(find "$SCRIPT_DIR/../experiments_results" -name "polymer_prediction_*" -type d | sort | tail -1)
